...
export function FloatingContactBar() {
  ...
  const items = [
    ...
    { href: CONTACT.tel, label: t.contact.call, icon: Phone, color: "btn-ink" },
    { href: CONTACT.sms, label: t.contact.sms, icon: MessageSquare, color: "btn-cream" },
  ];
  return (
    <div className="fixed bottom-3 inset-x-3 z-30 md:bottom-4 md:inset-x-0 md:flex md:justify-center pointer-events-none">
      <div className="bg-card/95 backdrop-blur rounded-full shadow-2xl border border-border/60 p-1.5 flex gap-1.5 pointer-events-auto md:gap-2">
        {items.map((it) => (
